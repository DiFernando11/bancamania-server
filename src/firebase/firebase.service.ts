...
export class FirebaseService {
  ...
  public async createCode({ data, feature }: CreateCode) {
    try {
      ...
      console.log(error, 'Error al crear el codigo');
  ...
  public async deleteCodeById(id: string): Promise<void> {
    try {
      ...
      await deleteDoc(docRef); // Borra el documento de la colección
      console.log(`El código con ID ${id} ha sido borrado exitosamente.`);
    } catch (error) {
      console.log(error);
  ...
  public async updateCodeExpireById(
    ...
    try {
      ...
      console.log(`El código con ID ${id} ha sido actualizado exitosamente.`);
    } catch (error) {
      console.log(error);
  ...
  // Método para eliminar documentos no expirados
  public async deleteNonExpiredCodes(): Promise<void> {
    try {
      const newDate = Timestamp.fromDate(new Date());

      // Crear consulta para buscar documentos no expirados
      ...
      querySnapshot.docs.forEach((docSnap: QueryDocumentSnapshot) => {
      ...
      const deletePromises = querySnapshot.docs.map(
        (docSnap: QueryDocumentSnapshot) => {
          ...
          console.log('Eliminando documento:', data); // Muestra los datos en consola
